...
export default function StockReason({ symbol }) {
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2.5">
          <Sparkles />
          <span className="font-semibold text-gray-900 text-sm">Why is this stock moving?</span>
          <PremiumBadge />
        </div>
    ...
    return (
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h3 className="font-semibold text-gray-900">Why is this stock moving?</h3>
            <PremiumBadge />
          </div>
          <button onClick={() => setExpanded(false)} title="Collapse" className="text-gray-400 hover:text-gray-600">
            <ChevronUp />
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 space-y-4">
      ...
      {data.headline && (
        <p className="text-sm font-medium text-gray-800">{data.headline}</p>
      )}
          ...
          <ul className="space-y-2">
            {data.news.slice(0, 3).map((n, i) => (
              <li key={i}>
                ...
                {n.source && (
                  <span className="text-xs text-gray-400 ml-1">— {n.source}</span>
                )}
              </li>
            ))}
          </ul>

...
function MetricCell({ label, value }) {
  return (
    <div className="bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-lg p-4 shadow-sm">
      <p className="text-xs text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-1">{label}</p>
      <p className="text-base font-semibold text-gray-900 dark:text-slate-100">
        {value ?? <span className="text-gray-400 dark:text-slate-500 font-normal">N/A</span>}
      </p>
    </div>
  );
}
...
export default function KeyMetrics({ data }) {
  ...
  return (
    <section>
      <h2 className="text-base font-semibold text-gray-800 dark:text-slate-200 mb-3">Key Metrics</h2>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        {metrics.map((m) => <MetricCell key={m.label} label={m.label} value={m.value} />)}
      </div>
    </section>
  );
}

...
export default function ProsConsList({ pros = [], cons = [] }) {
  ...
  return (
    <section>
      <h2 className="text-base font-semibold text-gray-800 dark:text-slate-200 mb-3">Pros & Cons</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <ul className="space-y-2">
              {pros.map((p, i) => (
                <li key={i} className="text-sm text-green-700 dark:text-green-400 flex items-start gap-2">
                  <span className="text-green-400 dark:text-green-500 mt-0.5 flex-shrink-0">•</span>
                  {p}
                </li>
              ))}
            </ul>
        ...
        {cons.length > 0 && (
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700/50 rounded-xl p-4">
            <h3 className="font-semibold text-red-800 dark:text-red-300 mb-3 flex items-center gap-2 text-sm">
